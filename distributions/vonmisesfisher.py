...
# below taken from https://github.com/nicola-decao/s-vae-pytorch/blob/master/hyperspherical_vae/distributions/von_mises_fisher.py
...
class VonMisesFisher(torch.distributions.Distribution):
    ...
    @property
    def variance(self):
        # TODO definitly wrong!
        return 1 / (self.scale + 1e-6)

    def __init__(self, loc, scale, validate_args=None, k=1, entropy_option=2):
    ...
    def _log_normalization(self):
        # TODO why -inf for rep_dim >>3 (deleted
        output = -(
                (self.__m / 2 - 1) * torch.log(self.scale)
                - (self.__m / 2) * math.log(2 * math.pi)
                - (self.scale + torch.log(ive(self.__m / 2-1, self.scale)+1)) #(self.scale + torch.log(ive(self.__m / 2 -1, self.scale)))
        )
...
def _kl_vmf_uniform(vmf, hyu):
    ...
    hyu_entropy = hyu.entropy().to(vmf_entropy.device)
